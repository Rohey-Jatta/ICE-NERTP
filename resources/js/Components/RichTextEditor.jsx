...
export default function RichTextEditor({ value, onChange, placeholder = 'Add your comments here...', minHeight = '200px' }) {
    ...
    const ToolButton = ({ title, onClick, children }) => (
        <button
            ...
            className="px-2 py-1 text-sm rounded hover:bg-slate-100 text-gray-200 transition-colors select-none"
        >
            {children}
        </button>
    );
    ...
    return (
        <div
            className="border border-slate-500 rounded-lg overflow-hidden bg-white focus-within:border-teal-400 transition-colors shadow-lg"
            dir="ltr"
            style={{ direction: 'ltr', unicodeBidi: 'normal' }}
        >
            {/* Toolbar */}
            <div
                className="flex flex-wrap items-center gap-1 px-3 py-2 bg-white border-b border-slate-200"
                dir="ltr"
            >
                {/* Heading select */}
                <select
            ...
            <div
                ...
                className="px-4 py-3 text-iec-navy focus:outline-none"
                style={{
                    minHeight,
                    direction: 'ltr',
                    textAlign: 'left',
