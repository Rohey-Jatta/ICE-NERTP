...
const PARTY_STATUS_CFG = {
    accepted:                  { color: 'bg-iec-pink-500/20 text-iec-pink-600 border-teal-500/30',    icon: '✓', label: 'Accepted' },
    ...
    return (
        <AppLayout user={auth?.user}>
            <div className="container mx-auto px-4 py-8">
                <div className="mb-6">
                    <Link href="/chairman/dashboard" className="text-slate-500 hover:text-iec-navy text-sm inline-flex items-center gap-1 mb-3">
                        ← Chairman Dashboard
                    </Link>
                    <div className="flex flex-wrap gap-4 items-center justify-between">
                        <div>
                            <h1 className="text-3xl font-bold text-iec-navy">National Certification Queue</h1>
                            <p className="text-slate-500 mt-1 text-sm">Final approval authority — results that have passed all lower certification levels</p>
                        </div>
                        <div className={`px-5 py-2.5 rounded-xl font-bold text-lg border ${
                            pendingCount > 0
                                ? 'bg-amber-500/20 text-amber-300 border-amber-500/40'
                                : 'bg-iec-pink-500/20 text-iec-pink-600 border-teal-500/30'
                        }`}>
                ...
                {pendingResults.length === 0 ? (
                    <div className="bg-white rounded-xl p-16 border border-teal-500/20 text-center">
                        <div className="text-5xl mb-4">✅</div>
                        <h2 className="text-xl font-bold text-iec-navy mb-2">All Clear</h2>
                        <p className="text-slate-500 text-sm mb-4">No results pending national certification.</p>
                        <Link href="/chairman/all-results" className="inline-block px-5 py-2 bg-white hover:bg-slate-100 text-iec-navy rounded-lg text-sm">
                            View All Results →
                        </Link>
                    </div>
                ) : (
                    <div className="space-y-5">
                        {pendingResults.map((result) => {
                            ...
                            return (
                                <div key={result.id} className="bg-white rounded-xl border border-amber-500/30 overflow-hidden">
                                    {/* Header */}
                                    <div className="p-5 border-b border-slate-200">
                                        <div className="flex flex-wrap gap-4 items-start justify-between">
                                            <div>
                                                <div className="flex items-center gap-3 flex-wrap">
                                                    <h2 className="text-xl font-bold text-iec-navy">{result.polling_station_name}</h2>
                                            ...
                                            <button
                                                ...
                                                className="px-3 py-1.5 bg-white hover:bg-slate-100 text-slate-600 text-xs rounded-lg transition-colors"
                                            >
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-5">
                                        {[
                                            { label: 'Registered',  value: result.total_registered_voters?.toLocaleString(), color: 'text-slate-600' },
                                            { label: 'Votes Cast',  value: result.total_votes_cast?.toLocaleString(),         color: 'text-iec-navy' },
                                            { label: 'Turnout',     value: `${result.turnout_percentage}%`,                   color: 'text-iec-pink-600' },
                                    ...
                                    {/* Expanded details */}
                                    {isExpanded && (
                                        <div className="px-5 pb-5 space-y-5 border-t border-slate-200 pt-4">
                                                            ...
                                                            .map((cv, idx) => {
                                                                ...
                                                                return (
                                                                    <div key={idx} className="flex items-center gap-3">
                                                                        ...
                                                                        <span className="text-slate-600 text-sm w-36 truncate">
                                                                            {cv.candidate_name}
                                                                            {idx === 0 && <span className="ml-1 text-iec-pink-600 text-xs">🏆</span>}
                                                                        </span>
                                                                        <span className="text-slate-500 text-xs w-10">{cv.party_abbr}</span>
                                                                        ...
                                                                        <span className="text-iec-navy font-semibold text-sm w-16 text-right">{cv.votes?.toLocaleString()}</span>
                                            ...
                                            {result.party_acceptances?.length > 0 && (
                                                <div>
                                                    <div className="text-xs text-slate-500 uppercase tracking-wide mb-2">Party Decisions</div>
                                                    <div className="space-y-2">
                                                        {result.party_acceptances.map((pa, idx) => {
                                                            const cfg = PARTY_STATUS_CFG[pa.status] || { color: 'bg-slate-100 text-slate-600 border-slate-200', icon: '○', label: pa.status };
                                                            return (
                                                                <div key={idx}>
                                            ...
                                            {(result.ward_comments || result.constituency_comments || result.admin_area_comments) && (
                                                <div>
                                                    <div className="text-xs text-slate-500 uppercase tracking-wide mb-2">Previous Approver Notes</div>
                                                    <div className="space-y-2">
                                            ...
                                            {result.certification_chain?.length > 0 && (
                                                <div>
                                                    <div className="text-xs text-slate-500 uppercase tracking-wide mb-2">Certification Chain</div>
                                                    <div className="flex flex-wrap gap-2">
                                                        {result.certification_chain.map((c, idx) => (
                                                            <div key={idx} className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${
                                                                c.status === 'approved' ? 'bg-iec-pink-500/20 text-iec-pink-600' : 'bg-red-500/20 text-red-300'
                                                            }`}>
                                            ...
                                            {result.photo_url && (
                                                <div>
                                                    <div className="text-xs text-slate-500 uppercase tracking-wide mb-2">Result Sheet</div>
                                                    <button onClick={() => setPhotoOpen(result.photo_url)}
                                                        className="inline-flex items-center gap-2 text-iec-pink-600 hover:text-iec-pink-600 text-sm transition-colors">
                    ...
                    >
                        ...
                        <div className="px-6 pt-6 pb-4 flex-shrink-0 border-b border-slate-200">
                            <div className="flex items-start justify-between gap-4">
                                <h2 className="text-lg font-bold text-iec-navy">
                                    {actionResult.type === 'certify' ? '🏛️ Confirm National Certification' : '↩ Return to Admin Area'}
                                </h2>
                                <button onClick={() => setActionResult(null)}
                                    className="text-slate-500 hover:text-iec-navy text-2xl w-8 h-8 flex items-center justify-center flex-shrink-0">
                                    ×
                                </button>
                            </div>
                        </div>
            ...
            {photoOpen && (
                <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4" onClick={() => setPhotoOpen(null)}>
                    <div className="relative max-w-5xl w-full" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => setPhotoOpen(null)} className="absolute -top-10 right-0 text-iec-navy text-3xl hover:text-slate-600">×</button>
